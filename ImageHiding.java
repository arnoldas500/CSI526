...
class Steganography
{
 ...
 public void encode(byte[] wavByteArry, int encodeBits) throws FileNotFoundException   // encode will take in a byte array (text) instead of originally (BufferedImage)
 {
  ...
  for (int i = 0; i < imageRGB.length; i++)
  {
	  imageByte[i] = wavByteArry[i];
	 
	  //System.out.println("imageByte[i]: "+imageByte[i]);
	  int encodeData = (wavByteArry[i] & encodeMask) >>> (8 - encodeBits);   // Will Encode the text byte array..
